...
// 获取总项目数
$totalProjectsStmt = $pdo->query('SELECT COUNT(*) FROM projects');
$totalProjects = $totalProjectsStmt->fetchColumn();

// 设置每页显示数量和当前页码
$perPage = 10;
$page = (int)($_GET['page'] ?? 1);
$page = max(1, min($page, ceil($totalProjects / $perPage)));
$offset = ($page - 1) * $perPage;

// 查询当前页的项目数据
$stmt = $pdo->prepare('SELECT p.id, p.name, p.category, p.level, p.total_amount, p.manager_id, u.name AS manager_name, u.login_id AS manager_login_id,
...
<div class="container">
    ...
    <div class="card">
        ...
        <div class="pagination">
            <?php if ($page > 1): ?>
                <a href="<?= e(url_for('admin/projects.php')) ?>?page=<?= $page - 1 ?>" class="pagination-link">上一页</a>
            <?php endif; ?>
            ...
            if ($startPage > 1) {
                echo '<a href="' . e(url_for('admin/projects.php')) . '?page=1" class="pagination-link">1</a>';
                if ($startPage > 2) {
            ...
            ?>
                <a href="<?= e(url_for('admin/projects.php')) ?>?page=<?= $i ?>" class="pagination-link <?= $i === $page ? 'active' : '' ?>">
                    <?= $i ?>
                </a>
            <?php endfor; ?>
            ...
            <?php if ($endPage < $totalPages): ?>
                ...
                <?php endif; ?>
                <a href="<?= e(url_for('admin/projects.php')) ?>?page=<?= $totalPages ?>" class="pagination-link"><?= $totalPages ?></a>
            <?php endif; ?>
            
            <?php if ($page < $totalPages): ?>
                <a href="<?= e(url_for('admin/projects.php')) ?>?page=<?= $page + 1 ?>" class="pagination-link">下一页</a>
            <?php endif; ?>
        </div>
    </div>
</div>
