...
<div class="container">
    ...
    <div class="card">
        ...
        <form method="post" class="member-actions"<?= !$allocation_enabled ? ' disabled' : '' ?>>
            <input type="hidden" name="action" value="add_member">
            <div class="member-picker" style="max-width: 800px; margin: 0 auto;">
                <label class="member-search-label" for="member-search">搜索成员（支持姓名或工号部分匹配）</label>
                <input type="text" id="member-search" class="member-search" placeholder="输入关键字筛选"<?= !$allocation_enabled ? ' disabled' : '' ?>>
                <select name="user_id" id="member-select" required style="height: 42px !important; padding: 8px 12px !important; border: 1px solid rgba(15, 23, 42, 0.12); border-radius: 8px; background: white; font-size: 14px; cursor: pointer; line-height: 24px;"<?= !$allocation_enabled ? ' disabled' : '' ?>>
                ...
                <button type="submit"<?= !$allocation_enabled ? ' disabled' : '' ?>>添加成员</button>
            </div>
        </form>
                    ...
                    <table class="table">
                        ...
                        <?php foreach ($allocations as $allocation): ?>
                            <tr>
                                ...
                                <td>
                                    <button type="submit" name="remove" value="<?= (int)$allocation['id'] ?>" class="secondary" onclick="return confirm('确认删除该成员？');"<?= !$allocation_enabled ? ' disabled' : '' ?>>删除</button>
                                </td>
                            </tr>
                        <?php endforeach; ?>
                        </tbody>
